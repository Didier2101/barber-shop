      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 h-20 bg-surface/90 backdrop-blur-2xl border-b border-white/5 flex items-center justify-between px-6 z-[60]">
        ...
        <div className="relative" ref={mobileDropdownRef}>
          <button
            ...
            className="w-10 h-10 rounded-xl border border-[#f59e0b]/20 bg-black flex items-center justify-center overflow-hidden shrink-0"
          >
            {userProfile.avatar_url
          ...
          {isMobileUserDropdownOpen && (
            <div className="absolute top-full right-0 mt-3 w-48 bg-surface border border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="p-2 space-y-1">
                <Link
                  href={`/dashboard/barber/${barberId}/profile`}
                ...
                <button
                  onClick={() => { setIsMobileUserDropdownOpen(false); handleLogout(); }}
                  className="flex items-center gap-3 w-full px-4 py-3 rounded-xl text-xs font-bold text-red-500 hover:bg-red-500/10 transition-all"
                >
                  <LogOut size={16} />
                  Cerrar Sesión
